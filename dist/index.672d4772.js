...
function setActiveNav() {
    ...
    navbarLinks.forEach((link)=>{
        if (link.getAttribute("href").substring(1) === visibleSection) link.classList.add("active");
...
function toggleHamburgerMenu() {
    ...
    if (window.innerWidth < 768) navbar.classList.toggle("nav-open");
}
...
function removeNavOpen() {
    ...
    if (window.innerWidth > 768) {
        if (navElement && navElement.classList.contains("nav-open")) navElement.classList.remove("nav-open");
        if (hamburgerElement && hamburgerElement.classList.contains("active")) hamburgerElement.classList.remove("active");
    }
}
